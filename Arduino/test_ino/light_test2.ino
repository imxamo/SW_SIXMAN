...
void setup() {
  ...
  // ✅ 동기화 확인
  if (now < 100000) {
    Serial.println("❌ NTP 동기화 실패! 라우터/인터넷 확인 필요");
    Serial.println("시간이 부정확하므로 재부팅하세요.");
    while(1) { delay(1000); }  // 멈춤
  } else {
    Serial.println("✅ NTP 동기화 완료!");
    struct tm timeinfo;
    getLocalTime(&timeinfo);
    Serial.printf("현재 시각: %04d-%02d-%02d %02d:%02d:%02d\n",
  ...
  Serial.printf("last_check_time 초기화: %ld\n", last_check_time);
