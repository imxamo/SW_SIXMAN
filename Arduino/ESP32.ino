...
#define PUMP_IN2 17
#define PUMP_ENA 18
#define RELAY_PIN 23 //led
#define COOLING_FAN_PIN 13
#define BUTTON_PIN 4  // led 제어 버튼 핀
...
void setup() {
  ...
  // 버튼 핀 설정
  pinMode(BUTTON_PIN, INPUT_PULLUP); // 버튼은 풀업 방식으로 설정
...
void loop() {
  ...
  // 버튼 입력 처리
  if (digitalRead(BUTTON_PIN) == LOW) {  // 버튼이 눌리면
    delay(200);  // 디바운싱을 위한 지연 시간
    ledState = !ledState;  // LED 상태를 토글
    digitalWrite(RELAY_PIN, ledState);  // LED 상태 변경
    Serial.print("LED 상태: ");
